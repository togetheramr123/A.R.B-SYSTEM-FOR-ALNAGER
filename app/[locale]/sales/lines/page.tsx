...
export default async function SalesLinesPage(props: {
  params: Promise<{
    locale: string;
  }>;
  ...
  const formattedLines = lines.map((line: any) => {
    return {
      ...
      customerName: line.order.partner?.name || "-",
      quantity: line.qty?.toFixed(3) || "0.000",
  ...
  const columns = [{
    ...
    label: "إجمالي الكمية",
    width: "w-32",
    render: (row: any) => <span className="font-medium text-slate-700"> {row.quantity} </span>
  }, {
    key: "priceUnit",
    label: "سعر الوحدة",
    width: "w-32",
    render: (row: any) => <span className="font-medium text-slate-700"> {row.priceUnit} </span>
  }];
  const breadcrumbs = [{
    label: "المنتجات",
  ...
  if (product) {
    ...
    breadcrumbs.push({
      label: `سجل المبيعات لـ ${product.name}`,
      href: "#"
    });
  } else {
    breadcrumbs.push({
  ...
  return <div className="h-full flex flex-col pt-14 bg-slate-50" dir="rtl">
      ...
      <div className="p-4 flex-1 overflow-hidden h-full">
        {" "}
        <OdooListView title={product ? `سجل المبيعات لـ ${product.name}` : "سجل المبيعات"} columns={columns} data={formattedLines} />{" "}
      </div>{" "}
    </div>;
}