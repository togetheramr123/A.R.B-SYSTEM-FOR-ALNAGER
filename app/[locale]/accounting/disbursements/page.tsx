...
export default async function DisbursementsListPage(props: {
  params: Promise<{
    locale: string;
  }>;
}) {
  ...
  const payments = await getAllPayments("outbound");
  return <div className="p-4" dir="rtl">
      {" "}
      <TopPortal>
        {" "}
        <Link href={`/${locale}/accounting/payments/new?type=outbound`} className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-sm px-3 py-1 text-xs font-bold flex items-center gap-1.5 transition-colors">
          {" "}
      ...
      {payments.length === 0 ? <div className="bg-white border border-slate-200 rounded-lg p-12 text-center">
          ...
          <p className="text-slate-500 text-sm mb-4">
            أنشئ سند صرف جديد لتسجيل المبالغ المدفوعة للموردين
          </p>{" "}
          <Link href={`/${locale}/accounting/payments/new?type=outbound`} className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 text-sm font-medium inline-flex items-center gap-2">
            {" "}
          ...
          <table className="w-full text-right text-sm">
            ...
            <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
              ...
              <tr>
                ...
                <th className="py-2.5 px-4 font-bold">المورد</th>{" "}
            ...
            <tbody className="divide-y divide-slate-100">
              {" "}
              {payments.map((pay: any) => <tr key={pay.id} className="hover:bg-red-50/50 group cursor-pointer">
                  {" "}
                  <td className="py-2 px-4 text-slate-600">
                    {" "}
                    {new Date(pay.date).toLocaleDateString("en-CA")}{" "}
                  </td>{" "}
                  <td className="py-2 px-4 font-medium text-red-700">
                    {" "}
                  ...
                  <td className="py-2 px-4 font-bold text-red-700">
                    {" "}
