...
# Extract the old logic using string indexing to avoid regex escape issues
start_idx = content.find('const handleExport = async (format: "csv" | "xlsx", selectedFields: string[]) => {')
end_idx = content.find('// Selection Logic', start_idx)

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + real_export_logic + "\n  " + content[end_idx:]
    with open("components/inventory/ProductListClient.tsx", "w") as f:
        f.write(content)
    print("Replaced handleExport successfully")
else:
    print("Could not find old handleExport logic")
