...
const downloadPdf = async (design: '1' | '2' = '1') => {
  if (!initialData?.id) {
    toast.error('يرجى حفظ الأمر أولاً قبل الطباعة');
    return;
  }
  const toastId = toast.loading('جاري تحميل الـ PDF...');
  try {
    const { generatePdfFromPrintPage } = await import('@/lib/whatsappShare');
    const printUrl = `/${locale}/purchases/${initialData.id}/print?design=${design}`;
    const pdfBlob = await generatePdfFromPrintPage(printUrl);
    const fileName = `${orderName || initialData?.name || 'أمر_شراء'}.pdf`;
...
const openWhatsAppWithPdf = async () => {
  ...
  try {
    ...
    const printUrl = `/${locale}/purchases/${initialData?.id}/print?design=1`;
    const pdfBlob = await generatePdfFromPrintPage(printUrl);
                ...
                <div className="flex items-center gap-2">
                  ...
                  {watch('vendor') && (
                    <div className="relative">
                      <button type="button" onClick={() => setWhatsappMenuOpen(!whatsappMenuOpen)} disabled={whatsappLoading} className="flex items-center justify-center w-9 h-9 rounded-lg bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white transition-colors shadow-sm flex-shrink-0" title="واتساب">
