...
export function PurchasesHeader({
  ...
  /* Assuming Purchases namespace exists */
  ...
  return <ModuleNavPortal>
      ...
      <div className="flex items-center gap-1 h-full text-[13.5px] font-medium text-slate-700">
        ...
        <nav className="flex items-center h-full rtl:space-x-reverse">
          {" "}
          <Link onClick={() => useBreadcrumbStore.getState().reset()} href={`/${locale}/purchases/orders`} className={`px-3 flex items-center h-full hover:bg-slate-100 transition-colors ${isActive("/purchases/orders") ? "bg-slate-100 font-bold text-slate-900" : ""}`}>
            {" "}
            الطلبات{" "}
          </Link>{" "}
          <div className="relative group h-full flex items-center">
            ...
            <div className="absolute top-10 right-0 mt-0 w-48 bg-white shadow-sm border border-slate-200 rounded-sm py-1 hidden group-hover:block z-50">
              ...
              <Link onClick={() => useBreadcrumbStore.getState().reset()} href={`/${locale}/purchases/pricelists`} className="block px-4 py-2 hover:bg-slate-100 transition-colors text-right text-sm">
                {" "}
                قوائم أسعار الشراء{" "}
              </Link>{" "}
            </div>{" "}
          ...
          <div className="relative group h-full flex items-center">
            {" "}
            <button className={`px-3 flex items-center gap-1 h-full hover:bg-slate-100 transition-colors ${isActive("/reporting") || isActive("/partner_ledger") ? "bg-slate-100 font-bold text-slate-900" : ""}`}>
              {" "}
              إعداد التقارير{" "}
              <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 group-hover:rotate-180`} />{" "}
            </button>{" "}
            <div className="absolute top-10 right-0 mt-0 w-56 bg-white shadow-sm border border-slate-200 rounded-sm py-1 hidden group-hover:block z-50">
              {" "}
              <Link onClick={() => useBreadcrumbStore.getState().reset()} href={`/${locale}/purchases/reporting`} className="block px-4 py-2 hover:bg-slate-100 transition-colors text-right text-sm">
                {" "}
                تحليل المشتريات{" "}
              </Link>{" "}
          ...
          <Link onClick={() => useBreadcrumbStore.getState().reset()} href={`/${locale}/purchases/configuration`} className={`px-3 flex items-center h-full hover:bg-slate-100 transition-colors ${isActive("/configuration") ? "bg-slate-100 font-bold text-slate-900" : ""}`}>
            {" "}
            التهيئه{" "}
          </Link>{" "}
        </nav>{" "}
