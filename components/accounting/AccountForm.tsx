        ...
        <div className="mb-4 pr-4">
          <div className="w-1/2 mb-4">
            ...
            required: true
                  ...
                  })} <select {...register('allowedJournals')} className="flex-1 min-w-[20px] text-[13px] text-transparent focus:text-slate-900 outline-none bg-transparent appearance-none"> <option value="" className="text-slate-400"></option> {availableJournals?.filter(j => !watch('allowedJournals')?.includes(j.id)).map(j => <option key={j.id} value={j.id} className="text-slate-900">{j.name}</option>)} </select> </div> </div> </div> </div> {} <div className="space-y-4"> <div className="flex items-start"> <label className="text-[13px] text-slate-700 w-1/3 text-right pt-1 pl-2 font-medium">مهمل</label> <div className="w-2/3 flex items-center pt-1"> <input autoComplete="off" autoCorrect="off" spellCheck={false} type="checkbox" {...register('deprecated')} className="w-4 h-4 text-[#017E84] rounded border-slate-300" /> </div> </div> <div className="flex items-start"> <label className="text-[13px] text-slate-700 w-1/3 text-right pt-1 pl-2 font-medium">المجموعة <span className="text-[10px] text-blue-500 font-bold ml-1 cursor-help">?</span></label> <div className="w-2/3 border-b border-slate-300"> <input autoComplete="off" autoCorrect="off" spellCheck={false} disabled className="w-full text-[13px] bg-transparent outline-none text-slate-400 pb-1" /> </div> </div> </div> </div> </div> </div> {} 
