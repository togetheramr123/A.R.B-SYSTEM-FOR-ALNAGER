...
/** Separate Print dropdown button (like Odoo's) */
export function PrintMenu({ options }: { options: PrintOption[] }) {
  const [isOpen, setIsOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
  ...
  return <div className="relative" ref={menuRef}>
    ...
    {isOpen && <div className="absolute top-full rtl:right-0 ltr:left-0 mt-1 w-56 bg-white border border-slate-200 shadow-sm rounded-sm z-50 py-1">
      {options.map((opt, i) => (
        <button key={i} type="button" onClick={() => {
          setIsOpen(false);
          opt.onClick();
        }} className="w-full text-start px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 flex items-center gap-2">
          <Printer className="w-4 h-4 text-slate-400" /> {opt.label}
        </button>
      ))}
    </div>}
  </div>;
