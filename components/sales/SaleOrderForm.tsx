...
export function SaleOrderForm({
  ...
  const downloadSalePdf = async (design: '1' | '2' = '1') => {
    if (!initialData?.id) {
      toast.error('يرجى حفظ الأمر أولاً قبل الطباعة');
      return;
    }
    const loadingToast = toast.loading('جاري تحميل الـ PDF...');
    try {
      const { generatePdfFromPrintPage } = await import('@/lib/whatsappShare');
      const printUrl = `/${locale}/sales/${initialData.id}/print?design=${design}`;
      const pdfBlob = await generatePdfFromPrintPage(printUrl);
      const fileName = `${orderName || initialData?.name || 'عرض_سعر'}.pdf`;
      ...
      toast.success('تم تحميل الـ PDF بنجاح', { id: loadingToast });
    } catch (err: any) {
  ...
  const openWhatsAppChat = () => {
    ...
    const companyName = initialData?.company?.name || 'النجار للأدوات الصحية';
  ...
  const openWhatsAppWithPdf = async () => {
    ...
    try {
      const { generatePdfFromPrintPage, shareViaWhatsApp } = await import('@/lib/whatsappShare');
      const printUrl = `/${locale}/sales/${initialData?.id}/print?design=1`;
      const pdfBlob = await generatePdfFromPrintPage(printUrl);
      const pdfFileName = `${orderName || initialData?.name || 'عرض_سعر'}.pdf`;
      const companyName = initialData?.company?.name || 'النجار للأدوات الصحية';
      await shareViaWhatsApp({
        phone,
        pdfBlob,
        fileName: pdfFileName,
        greeting: `السلام عليكم ورحمة الله وبركاته\nمن شركة ${companyName}\nمرفق عرض السعر الخاص بكم`
      });
    } catch (err) {
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {watch('customer') && (
                      <div className="relative flex gap-1 shrink-0">
                        <button type="button" onClick={() => setWhatsappMenuOpen(!whatsappMenuOpen)} disabled={whatsappLoading} className="flex items-center justify-center w-9 h-9 rounded-lg bg-green-500 hover:bg-green-600 disabled:bg-green-300 text-white transition-colors shadow-sm" title="واتساب">
                        ...
                        {whatsappMenuOpen && (
                          <>
                            <div className="fixed inset-0 z-40" onClick={() => setWhatsappMenuOpen(false)} />
                            <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-lg shadow-lg z-50 min-w-[200px] overflow-hidden">
                              <button type="button" onClick={openWhatsAppChat} className="w-full text-right px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2 border-b border-slate-100">
                                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-green-500 flex-shrink-0"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2z"/></svg>
                                <span>💬 محادثة</span>
                              </button>
                              <button type="button" onClick={openWhatsAppWithPdf} className="w-full text-right px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 flex items-center gap-2">
                                <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-red-500 flex-shrink-0"><path d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm-1 7V3.5L18.5 9H13z"/></svg>
                                <span>📄 إرسال عرض السعر PDF</span>
                              </button>
                            </div>
                          </>
                        )}
